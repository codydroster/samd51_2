...
uint8_t gen1_init()
{
pGclk->GENCTRL[1].bit.GENEN = 1;	//gen enable
pGclk->GENCTRL[1].bit.SRC = 6;		//DPLL source
pGclk->GENCTRL[1].bit.DIV = 6;		//8Mhz output
while(pGclk->SYNCBUSY.bit.GENCTRL1);//GCLK_SYNCBUSY_GENCTRL1);	//while syncing
